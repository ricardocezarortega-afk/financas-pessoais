...
function initApp(){
  ...
  document.getElementById('root').innerHTML=`
    <div class="app">
      <aside class="sidebar">
        <div class="sidebar-logo"><div class="sidebar-logo-wrap"><div class="sidebar-logo-icon">💰</div><span class="sidebar-logo-name">FinançasPro</span></div></div>
        ...
        <div class="sidebar-footer"><button class="btn-logout" onclick="logout()"><i class="ti ti-logout"></i> Sair</button></div>
      </aside>
      <div class="main">
        <div class="topbar">
...
async function renderDashboard(){
  ...
  document.getElementById('content').innerHTML=`
    <div style="margin-bottom:6px;font-size:13px;color:var(--text2)">${nomeMes(mes,ano)} · <strong style="color:var(--text)">${(contas||[]).length} contas cadastradas</strong></div>
    <div class="cards-grid">
      <div class="card"><div class="card-icon icon-green"><i class="ti ti-trending-up"></i></div><div class="card-label">Receitas do mês</div><div class="card-value" style="color:var(--green-dark)">${fmt(receitas)}</div><div class="card-sub">Recebido este mês</div></div>
      <div class="card"><div class="card-icon icon-red"><i class="ti ti-trending-down"></i></div><div class="card-label">Despesas do mês</div><div class="card-value" style="color:var(--red)">${fmt(despesas)}</div><div class="card-sub">Gasto este mês</div></div>
      <div class="card"><div class="card-icon ${saldoMes>=0?'icon-green':'icon-red'}"><i class="ti ti-wallet"></i></div><div class="card-label">Saldo do mês</div><div class="card-value" style="color:${saldoMes>=0?'var(--green-dark)':'var(--red)'}">${fmt(saldoMes)}</div><div class="card-sub ${saldoMes>=0?'up':'down'}">${receitas>0?Math.round(saldoMes/receitas*100):0}% de economia</div></div>
      <div class="card"><div class="card-icon icon-blue"><i class="ti ti-building-bank"></i></div><div class="card-label">Saldo disponível</div><div class="card-value">${fmt(saldoContas)}</div><div class="card-sub">${(contas||[]).length} conta${(contas||[]).length!==1?'s':''}</div></div>
      <div class="card"><div class="card-icon icon-amber"><i class="ti ti-clock"></i></div><div class="card-label">A pagar</div><div class="card-value" style="color:var(--amber)">${fmt(apagar)}</div><div class="card-sub">Pendente no mês</div></div>
      <div class="card"><div class="card-icon icon-green"><i class="ti ti-cash"></i></div><div class="card-label">A receber</div><div class="card-value" style="color:var(--green-dark)">${fmt(areceber)}</div><div class="card-sub">Receitas futuras</div></div>
      <div class="card"><div class="card-icon icon-purple"><i class="ti ti-chart-line"></i></div><div class="card-label">Investimentos</div><div class="card-value" style="color:var(--purple)">${fmt(totalInvest)}</div><div class="card-sub">${(invest||[]).length} aplicação${(invest||[]).length!==1?'ões':''}</div></div>
      <div class="card"><div class="card-icon icon-blue"><i class="ti ti-building-estate"></i></div><div class="card-label">Patrimônio</div><div class="card-value">${fmtK(patrimonio)}</div><div class="card-sub">Contas + investimentos</div></div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
      <div>
        <div class="table-card">
          <div class="table-header"><span class="table-title">Últimos lançamentos</span><button class="btn" onclick="document.querySelector('[data-view=lancamentos]').click()">Ver todos</button></div>
          <table><thead><tr><th>Descrição</th><th>Categoria</th><th>Valor</th><th>Status</th></tr></thead><tbody id="tbody-recentes"></tbody></table>
        </div>
      </div>
      <div>
        <div class="table-card">
        ...
        ${(metas||[]).length?'<div class="chart-card" style="margin-top:14px"><div class="chart-title">Metas financeiras</div><div id="metas-dash"></div></div>':''}
      </div>
    </div>`;
  ...
  document.getElementById('tbody-apagar').innerHTML=proximas.length?proximas.map(l=>{
    ...
    const alerta=diff<0?'badge-red':diff===0?'badge-amber':diff<=3?'badge-amber':'badge-gray';
  ...
  if((metas||[]).length){
    document.getElementById('metas-dash').innerHTML=(metas||[]).slice(0,3).map(m=>{
      ...
      return`<div style="margin-bottom:12px">
        <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:4px"><span style="font-weight:600">${m.nome}</span><span style="color:var(--text2)">${pct}%</span></div>
...
async function renderLancamentos(){
  document.getElementById('topbar-actions').innerHTML=`<button class="btn btn-green" onclick="novoLancamentoModal()"><i class="ti ti-plus"></i> Novo lançamento</button>`;
  ...
  const rows=(lanc||[]).map(l=>`
    <tr>
      ...
      <td><div style="font-weight:600">${l.descricao}</div>${l.parcela_atual?'<div style="font-size:11px;color:var(--text2)">'+l.parcela_atual+'/'+l.total_parcelas+'x</div>':''}</td>
  ...
  document.getElementById('content').innerHTML=`
    <div class="table-card">
      <div class="table-header"><span class="table-title">Todos os lançamentos</span><span style="font-size:13px;color:var(--text2)">${(lanc||[]).length} registros</span></div>
      <table><thead><tr><th>Data</th><th>Descrição</th><th>Categoria</th><th>Conta/Forma</th><th>Valor</th><th>Status</th><th></th></tr></thead>
      <tbody>${rows||'<tr><td colspan="7" class="empty-row">Nenhum lançamento. Clique em + para adicionar.</td></tr>'}</tbody></table>
    </div>`;
}
  ...
  document.body.insertAdjacentHTML('beforeend',`
    <div class="overlay" id="overlay">
      <div class="modal" style="max-width:420px">
        ...
        <div class="tipo-grid">
          <div class="tipo-btn despesa selected" id="tipo-despesa" onclick="selectTipo('despesa')"><i class="ti ti-trending-down" style="color:var(--red)"></i><span>Despesa</span></div>
          <div class="tipo-btn receita" id="tipo-receita" onclick="selectTipo('receita')"><i class="ti ti-trending-up" style="color:var(--green-dark)"></i><span>Receita</span></div>
        </div>
...
async function selectTipo(tipo){
  ...
  document.getElementById('form-lancamento').innerHTML=`
    <div class="form-row">
      ...
      <div class="form-group"><label class="form-label">Valor (R$) *</label><input class="form-control" type="number" id="f_valor" placeholder="0,00" step="0.01"/></div>
    </div>
    <div class="form-group"><label class="form-label">Descrição *</label><input class="form-control" id="f_desc" placeholder="Ex: Supermercado, Salário..."/></div>
    <div class="form-row">
      <div class="form-group"><label class="form-label">Categoria</label><select class="form-control" id="f_cat"><option value="">Selecione...</option>${catOpts}</select></div>
      <div class="form-group"><label class="form-label">Status</label><select class="form-control" id="f_status">${statusOpts}</select></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label class="form-label">Conta</label><select class="form-control" id="f_conta"><option value="">Selecione...</option>${contaOpts}</select></div>
    ...
    ${tipo==='despesa'?'<div class="form-row"><div class="form-group"><label class="form-label">Parcelas</label><input class="form-control" type="number" id="f_parcelas" placeholder="1" min="1" max="60"/></div><div class="form-group"><label class="form-label">Vencimento</label><input class="form-control" type="date" id="f_venc" value="'+today()+'"/></div></div>':''}
    <div class="form-group"><label class="form-label">Observação</label><input class="form-control" id="f_obs" placeholder="Opcional"/></div>
...
async function salvarLancamento(){
  ...
  const base={user_id:uid(),tipo:tipoAtual,descricao:desc,valor,data,categoria:document.getElementById('f_cat').value,status:document.getElementById('f_status').value,conta_id:document.getElementById('f_conta').value||null,forma_pagamento:document.getElementById('f_forma').value,observacao:document.getElementById('f_obs').value,data_vencimento:document.getElementById('f_venc')?.value||data,total_parcelas:parcelas};
  if(parcelas>1){
    const inserts=[];
...
async function renderContas(){
  ...
  document.getElementById('content').innerHTML=`
    <div class="card" style="margin-bottom:20px;display:inline-block;min-width:220px">
      <div class="card-label">Saldo total disponível</div><div class="card-value">${fmt(total)}</div>
      ...
      ${(contas||[]).length?(contas||[]).map(c=>`
        <div class="conta-card">
          <div class="conta-info">
            ...
            <div><div class="conta-name">${c.nome}</div><div class="conta-type">${c.tipo||'Conta'}${c.banco?' · '+c.banco:''}</div></div>
          </div>
  ...
  document.body.insertAdjacentHTML('beforeend',`
    <div class="overlay" id="overlay">
      <div class="modal">
        ...
        <div class="form-row">
          <div class="form-group"><label class="form-label">Nome *</label><input class="form-control" id="f_nome" placeholder="Ex: Nubank"/></div>
          <div class="form-group"><label class="form-label">Banco</label><input class="form-control" id="f_banco"/></div>
        </div>
        <div class="form-row">
          <div class="form-group"><label class="form-label">Tipo</label><select class="form-control" id="f_tipo"><option>Conta corrente</option><option>Conta digital</option><option>Conta poupança</option><option>Dinheiro</option><option>Carteira</option></select></div>
        ...
        <div class="modal-footer"><button class="btn" onclick="closeModal()">Cancelar</button><button class="btn btn-green" onclick="salvarConta()"><i class="ti ti-check"></i> Salvar</button></div>
      </div>
    </div>`);
}
...
async function renderCartoes(){
  ...
  document.getElementById('content').innerHTML=`<div id="lista-cartoes">
    ${(cartoes||[]).length?(cartoes||[]).map(c=>{
      ...
      return'<div class="card" style="margin-bottom:14px"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><div><div style="font-size:16px;font-weight:700">'+c.nome+'</div><div style="font-size:12px;color:var(--text2)">'+(c.banco||'')+' · Fecha dia '+c.dia_fechamento+' · Vence dia '+c.dia_vencimento+'</div></div><button class="btn" onclick="deletarCartao(\''+c.id+'\')" style="padding:6px"><i class="ti ti-trash"></i></button></div><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:12px"><div><div style="font-size:11px;color:var(--text2);text-transform:uppercase">Limite total</div><div style="font-weight:700">'+fmt(limite)+'</div></div><div><div style="font-size:11px;color:var(--text2);text-transform:uppercase">Utilizado</div><div style="font-weight:700;color:var(--red)">'+fmt(usado)+'</div></div><div><div style="font-size:11px;color:var(--text2);text-transform:uppercase">Disponível</div><div style="font-weight:700;color:var(--green-dark)">'+fmt(limite-usado)+'</div></div></div><div class="progress-bar"><div class="progress-fill" style="width:'+pct+'%;background:'+(pct>=90?'var(--red)':pct>=70?'var(--amber)':'var(--green)')+'"></div></div><div style="font-size:11px;color:var(--text2);margin-top:4px">'+pct+'% utilizado</div></div>';
  ...
  document.body.insertAdjacentHTML('beforeend',`
    <div class="overlay" id="overlay">
      <div class="modal">
        ...
        <div class="form-row">
          <div class="form-group"><label class="form-label">Nome *</label><input class="form-control" id="f_nome" placeholder="Ex: Nubank Roxinho"/></div>
          <div class="form-group"><label class="form-label">Banco</label><input class="form-control" id="f_banco"/></div>
        </div>
        <div class="form-row">
        ...
        <div class="form-row">
          <div class="form-group"><label class="form-label">Dia fechamento</label><input class="form-control" type="number" id="f_fecha" placeholder="25" min="1" max="31"/></div>
          <div class="form-group"><label class="form-label">Dia vencimento</label><input class="form-control" type="number" id="f_vence" placeholder="3" min="1" max="31"/></div>
        </div>
        <div class="modal-footer"><button class="btn" onclick="closeModal()">Cancelar</button><button class="btn btn-green" onclick="salvarCartao()"><i class="ti ti-check"></i> Salvar</button></div>
      </div>
    </div>`);
}
...
async function renderAPagar(){
  ...
  const rows=(lanc||[]).map(l=>{
    ...
    return'<tr><td>'+l.descricao+'</td><td>'+venc.toLocaleDateString('pt-BR')+'</td><td>'+(l.categoria||'—')+'</td><td><strong style="color:var(--red)">'+fmt(l.valor)+'</strong></td><td><span class="badge '+badge+'">'+label+'</span></td><td><button class="btn" onclick="pagarLancamento(\''+l.id+'\')" style="padding:5px 10px;font-size:12px"><i class="ti ti-check"></i> Pagar</button> <button class="btn" onclick="deletarLancamento(\''+l.id+'\')" style="padding:5px 8px;font-size:12px"><i class="ti ti-trash"></i></button></td></tr>';
  }).join('');
  document.getElementById('content').innerHTML=`
    ...
    <div class="table-card">
      <div class="table-header"><span class="table-title">Contas pendentes</span><button class="btn btn-green" onclick="novoLancamentoModal()"><i class="ti ti-plus"></i> Nova despesa</button></div>
      <table><thead><tr><th>Descrição</th><th>Vencimento</th><th>Categoria</th><th>Valor</th><th>Status</th><th></th></tr></thead>
      <tbody>${rows||'<tr><td colspan="6" class="empty-row">Nenhuma conta pendente</td></tr>'}</tbody></table>
    </div>`;
}
...
async function renderParcelamentos(){
  ...
  (lanc||[]).forEach(l=>{
    ...
    if(!grupos[key])grupos[key]={items:[],pago:0,parcelas:l.total_parcelas};
  ...
  const html=Object.entries(grupos).map(([nome,g])=>{
    ...
    return'<div class="meta-card"><div class="meta-header"><span class="meta-title">'+nome+'</span><span class="meta-values">'+g.pago+' de '+g.parcelas+' pagas</span></div><div class="progress-bar"><div class="progress-fill" style="width:'+pct+'%;background:'+(pct>=100?'var(--green)':'var(--blue)')+'"></div></div><div style="display:flex;justify-content:space-between;font-size:12px;color:var(--text2);margin-top:6px"><span>Restante: <strong style="color:var(--red)">'+fmt(valor-pago)+'</strong></span><span>Total: '+fmt(valor)+'</span></div></div>';
  }).join('');
  document.getElementById('content').innerHTML='<div style="margin-bottom:16px;font-size:14px;color:var(--text2)">'+Object.keys(grupos).length+' parcelamento'+(Object.keys(grupos).length!==1?'s':'')+' em andamento</div>'+(html||'<div style="text-align:center;padding:60px;color:var(--text3)">Nenhum parcelamento ativo.</div>');
}
...
async function renderOrcamento(){
  ...
  const ini=`${ano}-${String(mes).padStart(2,'0')}-01`;
  const fim=`${ano}-${String(mes).padStart(2,'0')}-31`;
  const [{data:orc},{data:lanc}]=await Promise.all([sb.from('orcamentos').select('*').eq('user_id',uid()),sb.from('lancamentos').select('categoria,valor').eq('user_id',uid()).eq('tipo','despesa').gte('data',ini).lte('data',fim)]);
  const gastos={};
  ...
  const html=(orc||[]).map(o=>{
    ...
    return'<div class="meta-card"><div class="meta-header"><span class="meta-title">'+o.categoria+'</span><span style="font-size:12px;font-weight:700;color:'+cor+'">'+pct+'%</span></div><div class="progress-bar"><div class="progress-fill" style="width:'+pct+'%;background:'+cor+'"></div></div><div style="display:flex;justify-content:space-between;font-size:12px;color:var(--text2);margin-top:6px"><span>Gasto: <strong>'+fmt(gasto)+'</strong></span><span>Limite: '+fmt(o.limite)+'</span></div>'+(pct>=100?'<div style="font-size:12px;color:var(--red);margin-top:4px;font-weight:600">Limite atingido!</div>':'')+'</div>';
  }).join('');
  document.getElementById('content').innerHTML=html||'<div style="text-align:center;padding:60px;color:var(--text3)">Nenhum limite definido. Clique em + para começar.</div>';
}
  ...
  document.body.insertAdjacentHTML('beforeend',`
    <div class="overlay" id="overlay">
      <div class="modal">
        ...
        <div class="form-group"><label class="form-label">Categoria *</label><select class="form-control" id="f_cat"><option value="">Selecione...</option>${CATS_DESPESA.map(c=>'<option>'+c+'</option>').join('')}</select></div>
        <div class="form-group"><label class="form-label">Limite mensal (R$) *</label><input class="form-control" type="number" id="f_limite" placeholder="0" step="0.01"/></div>
        <div class="modal-footer"><button class="btn" onclick="closeModal()">Cancelar</button><button class="btn btn-green" onclick="salvarOrcamento()"><i class="ti ti-check"></i> Salvar</button></div>
      </div>
    </div>`);
}
...
async function renderMetas(){
  ...
  const html=(metas||[]).map(m=>{
    ...
    return'<div class="meta-card"><div class="meta-header"><span class="meta-title" style="font-size:15px">'+m.nome+'</span><div style="display:flex;gap:8px"><button class="btn" onclick="atualizarMeta(\''+m.id+'\','+m.valor_atual+','+m.valor_desejado+')" style="padding:4px 8px;font-size:12px"><i class="ti ti-edit"></i></button><button class="btn" onclick="deletarMeta(\''+m.id+'\')" style="padding:4px 8px;font-size:12px"><i class="ti ti-trash"></i></button></div></div><div style="font-size:13px;color:var(--text2);margin-bottom:8px">Prazo: '+prazo+'</div><div class="progress-bar" style="height:12px"><div class="progress-fill" style="width:'+pct+'%;background:'+(pct>=100?'var(--green)':pct>=70?'var(--amber)':'var(--blue)')+'"></div></div><div style="display:flex;justify-content:space-between;font-size:13px;margin-top:8px"><span><strong style="color:var(--green-dark)">'+fmt(m.valor_atual)+'</strong> <span style="color:var(--text2)">guardado</span></span><span style="font-weight:700;color:'+(pct>=100?'var(--green)':'var(--text)')+'">'+pct+'%</span><span style="color:var(--text2)">Meta: <strong>'+fmt(m.valor_desejado)+'</strong></span></div></div>';
  }).join('');
  ...
  document.body.insertAdjacentHTML('beforeend',`
    <div class="overlay" id="overlay">
      <div class="modal">
        ...
        <div class="form-group"><label class="form-label">Nome *</label><input class="form-control" id="f_nome" placeholder="Ex: Viagem Europa"/></div>
        <div class="form-row">
        ...
        <div class="modal-footer"><button class="btn" onclick="closeModal()">Cancelar</button><button class="btn btn-green" onclick="salvarMeta()"><i class="ti ti-check"></i> Salvar</button></div>
      </div>
    </div>`);
}
  ...
  document.body.insertAdjacentHTML('beforeend',`
    <div class="overlay" id="overlay">
      <div class="modal" style="max-width:360px">
        ...
        <div class="modal-footer"><button class="btn" onclick="closeModal()">Cancelar</button><button class="btn btn-green" onclick="salvarAtualizacaoMeta('${id}')"><i class="ti ti-check"></i> Salvar</button></div>
      </div>
    </div>`);
}
...
async function renderInvestimentos(){
  ...
  const rows=(invest||[]).map(i=>{
    ...
    return'<tr><td><strong>'+i.nome+'</strong></td><td><span class="badge badge-purple">'+(i.categoria||'—')+'</span></td><td>'+(i.instituicao||'—')+'</td><td>'+fmt(inv)+'</td><td><strong>'+fmt(atu)+'</strong></td><td style="color:'+(diff>=0?'var(--green-dark)':'var(--red)')+';font-weight:600">'+(diff>=0?'+':'')+pct+'%</td><td><button class="btn" onclick="atualizarInvestimento(\''+i.id+'\','+atu+')" style="padding:4px 8px;font-size:12px"><i class="ti ti-edit"></i></button> <button class="btn" onclick="deletarInvestimento(\''+i.id+'\')" style="padding:4px 8px;font-size:12px"><i class="ti ti-trash"></i></button></td></tr>';
  }).join('');
  document.getElementById('content').innerHTML=`
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:20px">
    ...
    <div class="table-card">
      ...
      <table><thead><tr><th>Nome</th><th>Categoria</th><th>Instituição</th><th>Investido</th><th>Atual</th><th>Rentab.</th><th></th></tr></thead>
      <tbody>${rows||'<tr><td colspan="7" class="empty-row">Nenhum investimento cadastrado.</td></tr>'}</tbody></table>
    </div>`;
}
    ...
    <div class="overlay" id="overlay">
      <div class="modal">
        ...
        <div class="form-row">
          ...
          <div class="form-group"><label class="form-label">Instituição</label><input class="form-control" id="f_inst" placeholder="Ex: Nubank, XP"/></div>
        </div>
        <div class="form-row">
          <div class="form-group"><label class="form-label">Categoria</label><select class="form-control" id="f_cat">${cats.map(c=>'<option>'+c+'</option>').join('')}</select></div>
          <div class="form-group"><label class="form-label">Data</label><input class="form-control" type="date" id="f_data" value="${today()}"/></div>
        </div>
        <div class="form-row">
          ...
          <div class="form-group"><label class="form-label">Valor atual (R$)</label><input class="form-control" type="number" id="f_atual" placeholder="Igual ao investido" step="0.01"/></div>
        </div>
        <div class="modal-footer"><button class="btn" onclick="closeModal()">Cancelar</button><button class="btn btn-green" onclick="salvarInvestimento()"><i class="ti ti-check"></i> Salvar</button></div>
      </div>
    </div>`);
}
...
async function salvarInvestimento(){
  ...
  if(!nome||!inv){alert('Nome e valor são obrigatórios');return;}
  ...
  document.body.insertAdjacentHTML('beforeend',`
    <div class="overlay" id="overlay">
      <div class="modal" style="max-width:360px">
        ...
        <div class="form-group"><label class="form-label">Valor atual (R$)</label><input class="form-control" type="number" id="f_atual" value="${atual}" step="0.01"/></div>
        <div class="modal-footer"><button class="btn" onclick="closeModal()">Cancelar</button><button class="btn btn-green" onclick="salvarAtualizacaoInv('${id}')"><i class="ti ti-check"></i> Salvar</button></div>
      </div>
    </div>`);
}
...
async function renderRelatorios(){
  ...
  document.getElementById('content').innerHTML=`
    <div class="card" style="margin-bottom:20px;background:linear-gradient(135deg,#0F172A,#1E3A5F);color:#fff">
      ...
      <div style="font-size:14px;opacity:.85;line-height:1.7">Neste mês você <strong>recebeu ${fmt(receitas)}</strong> e <strong>gastou ${fmt(despesas)}</strong>. ${economia>=0?'Saldo positivo de <strong>'+fmt(economia)+'</strong>.':'Ficou <strong>'+fmt(Math.abs(economia))+'</strong> no negativo.'}</div>
    </div>
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:20px">
      ...
      <div class="card"><div class="card-label">Maior categoria</div><div class="card-value" style="font-size:16px">${topCat?topCat[0]:'—'}</div><div class="card-sub">${topCat?fmt(topCat[1]):''}</div></div>
    ...
    <div class="charts-row">
      ...
      <div class="chart-card">
        ...
        <div id="cat-lista">${catOrdenada.map(([cat,val])=>'<div style="margin-bottom:10px"><div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:4px"><span style="font-weight:500">'+cat+'</span><span style="font-weight:700">'+fmt(val)+' <span style="color:var(--text2);font-weight:400">('+( despesas>0?Math.round(val/despesas*100):0)+'%)</span></span></div><div class="progress-bar"><div class="progress-fill" style="width:'+(despesas>0?Math.round(val/despesas*100):0)+'%;background:'+(CAT_COLORS[cat]||'#9CA3AF')+'"></div></div></div>').join('')}</div>
      </div>
    </div>`;
